...
export default function App() {
  ...
  // Check URL query parameters for public e-sign token
  useEffect(() => {
  ...
  const handleCopySigningLink = (token: string) => {
    ...
    showToast('Đã sao chép liên kết ký hợp đồng! Hãy dán gửi cho khách qua Zalo/SMS.');
  };
  ...
  // -------------------------------------------------------------
  // PUBLIC TENANT SIGNING VIEW (WHEN LINK IS OPENED ON PHONE)
  // -------------------------------------------------------------
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 py-8 px-4 flex items-center justify-center">
        <div className="max-w-2xl w-full bg-white text-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
          ...
          {publicSignSuccess ? (
            /* SUCCESS STATE + VIETQR DEPOSIT CODE */
            ...
            /* SIGNING FORM WITH TERMS & TOUCH CANVAS */
            <div className="space-y-5 text-xs text-slate-700">
              ...
              <div className="pt-2">
                <SignaturePad
                  onSave={(dataUrl) => handleTenantSignSubmit(publicSignToken, dataUrl, true, true)}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 max-w-lg w-full shadow-2xl text-slate-200 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-2 border-b border-slate-800">
              ...
              <button onClick={() => setShowCreateContractModal(false)} className="text-slate-400 hover:text-white font-bold">✕</button>
            </div>
            ...
            <form onSubmit={handleCreateContractSubmit} className="space-y-3.5 text-xs">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="button" onClick={() => setShowCreateContractModal(false)} className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold transition">
                  Hủy
                </button>
              </div>
            </form>
